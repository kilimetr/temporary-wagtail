...
class MenuItem(Orderable):
	...
	link_image      = models.ForeignKey("wagtailimages.Image", blank = False, null = True, on_delete = models.SET_NULL, related_name = "+")
	show_when       = models.CharField(max_length=15, choices=[
			('always', ("Always")), ('logged_in', ("When logged in")), ('not_logged_in', ("When not logged in"))
		], default='always', )

	page = ParentalKey("Menu", related_name = "menu_polozky")
...
class Menu(ClusterableModel):
	...
	panels = [
		MultiFieldPanel([
			...
			FieldPanel("slug")],
			heading = "Menu"),
		InlinePanel("menu_polozky", label = "Položka v menu")
	]
